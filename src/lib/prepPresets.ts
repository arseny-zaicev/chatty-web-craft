...
export const PREP_PRESETS: PrepPreset[] = [
  ...
  {
    id: "marketing_per_row_name_company",
    name: "Marketing Per-row - Name + Company",
    campaignType: "marketing",
    blurb: "Name AND company change per recipient. Use when the template references the lead's own company (e.g. \"Hey {{1}}, saw {{2}} is hiring...\").",
    recommendedFor: "Marketing templates where {{1}} = first name and {{2}} = the recipient's company name (both per row).",
    requiredSourceFields: ["phone", "first_name", "company"],
    optionalSourceFields: ["city", "industry"],
    variables: [
      { key: "var_1", kind: "per_row", description: "First name", example: "Layla", source: "first_name", fallback: "there" },
  ...
  {
    id: "marketing_per_row_name_company_industry",
    name: "Marketing Per-row - Name + Company + Industry",
    campaignType: "marketing",
    blurb: "Name, company AND industry all change per recipient. Use when the template references both the lead's company and their industry.",
    recommendedFor: "Marketing templates where {{1}} = first name, {{2}} = company, {{3}} = industry (all per row).",
    requiredSourceFields: ["phone", "first_name", "company", "industry"],
    optionalSourceFields: ["city"],
    variables: [
      { key: "var_1", kind: "per_row", description: "First name", example: "Layla", source: "first_name", fallback: "there" },
      { key: "var_2", kind: "per_row", description: "Company name", example: "Acme Trading", source: "company", fallback: "your company" },
      { key: "var_3", kind: "per_row", description: "Industry", example: "retail", source: "industry", fallback: "your industry" },
    ],
  },
  {
    id: "marketing_per_row_name_company_static",
    name: "Marketing Per-row - Name + Company + 1 static",
    campaignType: "marketing",
    blurb: "Name (per row) + Company (per row) + 1 campaign-static line pasted from Materials.",
    recommendedFor: "Marketing templates with {{1}} = name, {{2}} = the recipient's company, {{3}} = same pitch line for everyone.",
    requiredSourceFields: ["phone", "first_name", "company"],
    optionalSourceFields: ["city", "industry"],
    variables: [
      { key: "var_1", kind: "per_row", description: "First name", example: "Layla", source: "first_name", fallback: "there" },
