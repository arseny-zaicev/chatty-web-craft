...
const StageColumn = ({
  ...
  const commit = async () => {
    setEditing(false);
    if (onRename) await onRename(draft);
  };
  return (
    <div
      ref={setNodeRef}
    ...
    >
      <div className="px-3 py-2.5 border-b border-border flex items-center justify-between gap-2 group">
            ...
            <>
              <Input
                ...
                onKeyDown={(e) => {
                  ...
                  else if (e.key === "Escape") { setEditing(false); setDraft(stage.name); }
                }}
                className="h-7 text-sm py-1 px-2"
                autoFocus
              />
              <button onClick={commit} className="p-1 text-emerald-600 hover:text-emerald-500" title="Save"><Check className="w-3.5 h-3.5" /></button>
              <button onClick={() => { setEditing(false); setDraft(stage.name); }} className="p-1 text-muted-foreground hover:text-foreground" title="Cancel"><X className="w-3.5 h-3.5" /></button>
            </>
          ) : (
            <>
